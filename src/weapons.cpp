...
void WeaponSimpleGun::OnLeftMBPressed(double playerx, double playery){
    ...
    for(int i = 0; i < bullet_amt; i ++){
        ...
        EntityBuilder().AddComponent(new DrawComponent(bullet_texture,(angle * 360 )/ (2 * PI)))
                        ...
                        .AddComponent(new BulletComponent(angle,bullet_damage,bullet_speed + (rand() % (2*bullet_speed_variation)) - bullet_speed_variation ,bullet_range + (rand() % (2*bullet_range_variation)) - bullet_range_variation))
                        .Build();
    }
